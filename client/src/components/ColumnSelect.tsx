...
function SelectColumns({
  ...
  const setFocus = (i: any) => {
    if (!parentRef.current) {
      return;
    }

    (
      parentRef.current!.querySelectorAll('input')[i] as HTMLInputElement
    ).focus();
  ...
  const newColumnName = (column: any, i: any) => {
    var newColName = (
      parentRef.current!.querySelectorAll('input')[i] as HTMLInputElement
    ).value;
  ...
  const handleColumnClick = (id: any, column: any) => {
  ...
  return (
    <>
      <select
        value={defaultOption}
        onChange={(event) => setDefaultOption(event.target.value)}
      >
        <option
          value=""
          disabled
          ...
          {columns.map((column, i) => (
            <div
              ...
              className={`${styles.columnContainer} ${
                isChecked[`${i}`] ? styles.editable : ''
              } ${dropdownText == column ? styles.editable : ''}`}
            >
              <span
                className={`${styles.checkbox} ${
                  isChecked[`${i}`] ? styles.checkboxActive : ''
                } ${dropdownText == column ? styles.checkboxActive : ''}`}
                onClick={() => handleColumnClick(i, column)}
              >
                <div
                  className={`${
                    isChecked[`${i}`] || dropdownText == column
                      ? styles.tickActive
                      : styles.tickNotActive
                  } `}
                >
                  <TickSquare
                    size="23"
              ...
              <div
                className={`${styles.editColumn} ${
                  isChecked[`${i}`] ? styles.editColumnButton : ''
                }`}
                onClick={() => {
                  isChecked[`${i}`] || dropdownText == column
                    ? setFocus(i)
              ...
              <input
                className={`${styles.editName} ${
                  isEditing[`${i}`] && isChecked[`${i}`] ? styles.editing : ''
                }`}
                type="text"
                placeholder={column}
                name="customName"
                onChange={() => newColumnName(column, i)}
              ></input>
              ...
              <div className={`${styles.dropdownInfo} ${styles.triangle}`}>
